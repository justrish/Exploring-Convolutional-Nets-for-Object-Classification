...
int main( int argc, char** argv )
{
    ...
    if (ftrain.is_open()) {
        ...
        while ( getline (ftrain,line) ) {
         ...
         ////Reading individual training images
            int index = line.find(" ");
            label = atoi(line.substr(index+1,1).c_str());
            ...
            frameReference = cv::imread(videoin, CV_LOAD_IMAGE_COLOR);
            cv::Size refS = cv::Size((int) frameReference.cols,
                                      (int) frameReference.rows);
            ...
            std::string save_img;
            save_img.append(line);
            // replace all occurances of '/' so that no new directories are created
            replace(save_img.begin(), save_img.end(), '/', '_');
            ...
            drawKeypoints( frameUnderTest, keypoints_1, img_keypoints_1, cv::Scalar::all(-1), cv::DrawMatchesFlags::DEFAULT );
    ...
    /** @function readme */
    void readme()
    { std::cout << " Usage: ./pca_image <source_dir> <train dataset file>  <dump path> <class label>" << std::endl; }
    
    /*
     g++ -I/usr/include/opencv -I/usr/include/opencv2 -L/usr/local/lib/ -g -o pca_image  pca_image.cpp -lopencv_core -lopencv_imgproc -lopencv_highgui -lopencv_ml -lopencv_video -lopencv_features2d -lopencv_calib3d -lopencv_objdetect -lopencv_contrib -lopencv_legacy -lopencv_stitching -lopencv_features2d -lopencv_nonfree
     */
